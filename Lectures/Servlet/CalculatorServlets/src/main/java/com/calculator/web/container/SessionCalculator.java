...
public class SessionCalculator extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(op.equals("=")) {
			...
			int x = (Integer) session.getAttribute("value"); //이전에 servlet context에 저장되어 있는 값
			...
			session.setAttribute("value", v); //servlet Context에 값을 저장하게 된다 Map형식처럼
			session.setAttribute("op", op); //operatino도 저장할수 있다.
